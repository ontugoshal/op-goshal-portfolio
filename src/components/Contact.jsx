    ...
    <section>
      ...
      <div className="flex flex-col">
        {contactLinks.map((link, index) => (
          <div key={`${link.label}-${index}`} className="flex border-b border-dashed border-border-subtle last:border-b-0">
            <div className="w-[100px] sm:flex-1 flex items-center px-5 sm:px-10 py-5 border-r border-dashed border-border-subtle">
              <span className="text-text-primary font-bold text-[15px] sm:text-[17px] tracking-tight">{link.label}</span>
            </div>
            <div className="flex-1 flex items-center px-5 sm:px-10 py-5 overflow-hidden">
              <a 
                href={link.href} 
                target="_blank" 
